...
export function createPaginationMiddleware(options = {}) {
    const { defaultLimit = 50, maxLimit = 1000, pageKey = "page", limitKey = "limit", offsetKey = "offset", useOffset = false, } = options;
    return (next) => {
        return async function* (context) {
            const { metadata } = context.message;
            // Extract pagination params
            let page = metadata[pageKey];
            let limit = metadata[limitKey];
            let offset = metadata[offsetKey];
 ...
 * ```
 */
export async function* paginateAll(client, method, payload, options = {}) {
