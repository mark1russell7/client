...
function composeAbortSignals(...signals) {
    ...
    if ("any" in AbortSignal && typeof AbortSignal.any === "function") {
        try {
            const composedSignal = AbortSignal.any(validSignals);
            return {
                controller: { signal: composedSignal },
...
export function createOverallTimeoutMiddleware(options) {
    ...
    return (next) => {
        return async function* (context) {
            ...
            const { controller: composedController, cleanup } = composeAbortSignals(context.message.signal, timeoutController.signal);
 ...
 * ```
 */
export function createCombinedTimeoutMiddleware(options) {
    const { overall, perAttempt, message = "Request timeout" } = options;
    return (next) => {
        // Compose both timeout middlewares
        let composed = next;
        if (perAttempt) {
            const perAttemptMiddleware = createTimeoutMiddleware({ perAttempt, message: `${message} (per-attempt)` });
            composed = perAttemptMiddleware(composed);
        }
        if (overall) {
            const overallMiddleware = createOverallTimeoutMiddleware({ overall, message: `${message} (overall)` });
