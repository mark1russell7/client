 ...
 */
function defineError(code, meta) {
    const result = {
        code,
...
export const ERROR_REGISTRY = {
    ...
    [ErrorCode.NETWORK_ERROR]: defineError(ErrorCode.NETWORK_ERROR, {
        ...
        detail: "Failed to establish or maintain network connection. This typically indicates connectivity issues, DNS resolution failures, or network infrastructure problems.",
    ...
    CONNECTION_REFUSED: defineError(ErrorCode.CONNECTION_REFUSED, {
        ...
        detail: "The server actively refused the connection. This typically means the server is not running, the port is blocked, or a firewall is rejecting the connection.",
        category: ErrorCategory.NETWORK,
        retryable: true,
        severity: ErrorSeverity.ERROR,
    ...
    TIMEOUT: defineError(ErrorCode.TIMEOUT, {
        ...
        detail: "The request exceeded the configured timeout duration. This may indicate slow server response, network congestion, or overly aggressive timeout settings.",
...
export function httpStatusToErrorCode(status) {
    const match = Object.values(ERROR_REGISTRY).find((meta) => meta.httpStatus === status);
