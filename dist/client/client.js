 ...
 */
const CONTROL_FLOW_PROCEDURES = new Set([
    "chain",
    "parallel",
    "conditional",
    "tryCatch",
 ...
 */
function isControlFlowPath(path) {
...
export class Client {
     ...
     */
    static createChild(parent, additionalContext) {
        // Create a child with shared middleware chain
        const child = Object.create(Client.prototype);
     ...
     */
    withContext(context) {
        return Client.createChild(this, context);
    }
    /**
     ...
     * ```
     */
    async *stream(method, payload, options) {
        ...
        const effectiveContext = mergeContext(this.getEffectiveContext(), (opts.context ?? {}));
        ...
        if (opts.schema) {
            message.metadata.__schema = opts.schema;
        }
    ...
    schema(method, schemas) {
        ...
        if (!this.zodMiddleware) {
            throw new Error("Cannot register schema: createZodMiddleware() must be added via .use() first");
        }
        const key = methodToKey(method);
        this.zodMiddleware[SCHEMA_REGISTRY].set(key, schemas);
        return this;
    }
    /**
    ...
    findZodMiddleware() {
        ...
        return this.middleware.find((m) => SCHEMA_REGISTRY in m);
    }
    // ===========================================================================
    // Nested Route API
    // ===========================================================================
    ...
    async route(request) {
        ...
        if (!resolution.success) {
            ...
            const errorResults = resolution.errors.map((error) => [
                error.path,
                {
                    success: false,
                    error: {
            ...
            return buildResponse(errorResults);
        }
        // Create execution context
        const context = this.createExecutionContext(request);
        // Execute routes
        const batchConfig = request.batch ?? { strategy: "all" };
        const result = await executor.executeBatch(resolution.resolved, context, batchConfig);
    ...
    routeStream(request) {
        ...
        if (!resolution.success) {
            ...
            return {
                ...
                complete: Promise.resolve(buildResponse(errorResults)),
            };
        }
        const context = this.createExecutionContext(request);
        const streamConfig = request.batch ?? { strategy: "stream" };
        return executor.getStreamingResponse(resolution.resolved, context, streamConfig);
    }
    /**
    ...
    createExecutionContext(request) {
        ...
        const effectiveContext = mergeContext(this.getEffectiveContext(), (request.middlewares ?? {}));
        const context = {
            metadata: {
                ...this.defaultMetadata,
                ...effectiveContext,
     ...
     */
    async executeProcedure(resolved, context) {
