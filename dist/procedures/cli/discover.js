 ...
 */
async function discoverPackages(rootDir, verbose) {
    const discovered = [];
    ...
    async function scanPackage(packageName, nodeModulesPath) {
        if (scanned.has(packageName))
            return;
 ...
 */
function generateCode(packages) {
 ...
 */
async function loadFromRegistry(rootDir, verbose) {
    try {
...
export async function discover(options) {
    ...
    if (check) {
        ...
        try {
            ...
            const normalizeForCompare = (s) => s.replace(/Generated at: .+/, "Generated at: <timestamp>");
