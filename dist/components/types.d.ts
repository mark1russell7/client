 ...
 */
export type ComponentFactory<TData = unknown> = (ctx: ComponentContext<TData>) => ComponentOutput | Promise<ComponentOutput>;
/**
 * Streaming component factory (multi-yield).
 * Yields descriptors over time for live updates.
 */
export type StreamingComponentFactory<TData = unknown> = (ctx: ComponentContext<TData>) => AsyncGenerator<ComponentOutput, void, unknown>;
/**
 * Any component factory (single or streaming).
 */
export type AnyComponentFactory<TData = unknown> = ComponentFactory<TData> | StreamingComponentFactory<TData>;
/**
 ...
 */
export type InferComponentInput<T> = T extends ComponentDefinition<infer TData> ? TData : unknown;
/**
 * Infer whether a component is streaming.
 */
export type IsStreamingComponent<T> = T extends ComponentDefinition<unknown> ? T["streaming"] extends true ? true : false : false;
/**
 * Create a null output (render nothing).
 */
export declare function nullOutput(): NullOutput;
/**
 * Create a fragment output (multiple children without wrapper).
 */
export declare function fragment(...children: ComponentOutput[]): FragmentOutput;
/**
 * Check if output is a fragment.
 */
export declare function isFragment(output: AnyComponentOutput): output is FragmentOutput;
/**
 * Check if output is null.
 */
export declare function isNullOutput(output: AnyComponentOutput): output is NullOutput;
/**
 * Check if a factory is a streaming generator.
 */
export declare function isStreamingFactory<TData>(factory: AnyComponentFactory<TData>): factory is StreamingComponentFactory<TData>;
