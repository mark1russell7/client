 ...
 */
import { arrayList, arrayDeque, hashMap, 
// compose,
// boundedList,
// eventedList,
// eventedMap,
...
export function hashMapExample() {
    ...
    const aliceClone = { id: 1, name: "Alice Clone" };
...
export function readonlyExample() {
    ...
    const readonlyList = readonly()(mutableList);
    console.log("Get(0):", readonlyList.get(0)); // 1
    console.log("Size:", readonlyList.size); // 3
    try {
        readonlyList.add(4); // This will throw
    }
    catch (error) {
        console.log("Error:", error.message); // "Cannot modify readonly collection"
    }
    // Immutable factories
    emptyList();
...
export function factoriesExample() {
    ...
    // Create map
    const map = mapOf(["one", 1], ["two", 2], ["three", 3]);
