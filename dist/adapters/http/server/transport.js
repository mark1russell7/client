...
export class HttpServerTransport {
    ...
    constructor(server, options = {}) {
        ...
        if (options.app) {
            ...
            throw new Error("Express app is required. Please provide an app instance via options.app");
        }
        // Setup routes
        this.setupRoutes();
    }
    ...
    setupRoutes() {
        ...
        this.app.all(new RegExp(`^${this.options.basePath.replace(/\//g, "\\/")}\\/.*`), async (req, res) => {
     ...
     * Convert ServerResponse to HTTP response.
     */
    serverResponseToHttp(serverResponse, res) {
        ...
        if (typeof status.code === "string") {
            ...
            const mapped = ERROR_REGISTRY[status.code]?.httpStatus;
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                server.listen(this.options.port, this.options.host, () => {
                    console.log(`[${this.name}] Server listening on http://${this.options.host}:${this.options.port}`);
                    resolve();
                });
