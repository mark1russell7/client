 ...
 * ```
 */
export function createHttpRequestSchema<TBody>(
  z: ZodModule,
  bodySchema: ZodLike<TBody>,
): ZodLike<HttpRequest<TBody>> {
  return z.object({
    method: z.enum(["GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS"]),
    url: z.string(),
    headers: z.record(z.string()),
    body: bodySchema as any,
  }) as ZodLike<HttpRequest<TBody>>;
}
 ...
 * ```
 */
export function createHttpResponseSchema<TBody>(
  z: ZodModule,
  bodySchema: ZodLike<TBody>,
): ZodLike<HttpResponse<TBody>> {
  return z.object({
    status: z.number().int().min(100).max(599),
    statusText: z.string(),
    headers: z.record(z.string()),
    body: bodySchema as any,
  }) as ZodLike<HttpResponse<TBody>>;
}
 ...
 */
export type ExtractBody<T> = T extends HttpRequest<infer B>
  ? B
  : T extends HttpResponse<infer B>
    ? B
    : unknown;
