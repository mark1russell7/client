 ...
 */
export type ProcedurePaths<T, Prefix extends string[] = []> = T extends ProcedureDef
  ? Prefix
  : T extends ProcedureNamespace
  ? {
      [K in keyof T]: T[K] extends ProcedureDef
        ? [...Prefix, K & string]
        : T[K] extends ProcedureNamespace
        ? ProcedurePaths<T[K], [...Prefix, K & string]>
        : never;
    }[keyof T]
  : never;
 ...
 */
export type GetProcedure<T, P extends string[]> = P extends [infer K extends keyof T]
  ? T[K]
  : P extends [infer K extends keyof T, ...infer Rest extends string[]]
  ? GetProcedure<T[K], Rest>
  : never;

/**
 * Extract input type for a path
 */
export type PathInput<T, P extends string[]> = GetProcedure<T, P> extends ProcedureDef
  ? GetProcedure<T, P>["input"]
  : never;

/**
 * Extract output type for a path
 */
export type PathOutput<T, P extends string[]> = GetProcedure<T, P> extends ProcedureDef
  ? GetProcedure<T, P>["output"]
  : never;

/**
 * Base caller function type
 */
export type BaseCaller = <TOutput>(
  path: string[],
  input: unknown,
  options?: CallOptions
) => Promise<TOutput>;
...
export interface TypedCaller<T extends ProcedureNamespace> {
  <P extends ProcedurePaths<T> & string[]>(
    path: P,
    input: PathInput<T, P>,
    options?: CallOptions
  ): Promise<PathOutput<T, P>>;
}
...
export function createTypedCaller<T extends ProcedureNamespace>(
  ...
  return caller as TypedCaller<T>;
}
