 ...
 */
export type MiddlewareRunner<TContext, TReturn, TAsync extends boolean = false> =
  TAsync extends true
 ...
 */
export type AsyncMiddleware<TContextOut, TContextIn, TReturn> = Middleware<
  TContextOut,
  TContextIn,
  TReturn,
  true
 ...
 */
export type SyncMiddleware<TContextOut, TContextIn, TReturn> = Middleware<
  TContextOut,
  TContextIn,
  TReturn,
  false
 ...
 * ```
 */
export type MiddlewaresContext<
  TMiddlewares extends readonly Middleware<any, any, any, any>[],
  TContextIn = {},
> =
  // Recursive case: process next middleware in chain
  TMiddlewares extends readonly [
    Middleware<infer TNextContextOut, infer TNextContextIn, any, any>,
    ...infer TRest,
  ]
    ? // Check that the accumulated context satisfies next middleware's requirements
      TContextIn extends TNextContextIn
      ? // Check type constraint for recursion
        TRest extends readonly Middleware<any, any, any, any>[]
        ? // Recurse with accumulated context
          MiddlewaresContext<TRest, TContextIn & TNextContextOut>
        : never
      : // Requirements not met - incompatible middleware
        never
    : // Base case: no more middleware, return accumulated context
    TMiddlewares extends readonly []
    ? TContextIn
    : never;
 ...
 */
export type MiddlewareReturn<T> = T extends Middleware<any, any, infer R, any>
  ? R
  : never;
 ...
 */
export type MiddlewareContextIn<T> = T extends Middleware<any, infer Ctx, any, any>
  ? Ctx
  : never;
 ...
 */
export type MiddlewareContextOut<T> = T extends Middleware<infer Ctx, any, any, any>
  ? Ctx
  : never;
 ...
 */
export type IsAsyncMiddleware<T> = T extends Middleware<any, any, any, infer A>
  ? A
  : never;
