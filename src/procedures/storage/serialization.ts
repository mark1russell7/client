...
export async function deserializeProcedure(
  data: SerializedProcedure,
  options: DeserializeOptions = {}
): Promise<AnyProcedure> {
  ...
  if (data.handlerRef && options.handlerLoader) {
    try {
      ...
      if (handler && typeof handler === "function") {
        procedure.handler = handler as AnyProcedure["handler"];
      }
    } catch {
 ...
 */
export function deserializeProcedureSync(data: SerializedProcedure): AnyProcedure {
  const procedure: AnyProcedure = {
    path: data.path,
    input: anySchema,
    output: anySchema,
 ...
 */
export function getProcedureKey(pathOrProcedure: ProcedurePath | AnyProcedure): string {
 ...
 */
export function getSerializedKey(data: SerializedProcedure): string {
 ...
 */
export function serializeProcedures(
  procedures: AnyProcedure[],
  options: SerializeOptions = {}
): Array<[string, SerializedProcedure]> {
 ...
 */
export async function deserializeProcedures(
  items: SerializedProcedure[],
  options: DeserializeOptions = {}
): Promise<AnyProcedure[]> {
